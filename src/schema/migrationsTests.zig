...
test "addProperty(indexed) backfills the value index for existing rows" {
    ...
    try query.where(&readTransaction, readTransaction.root(), .{ .predicate = .{ .comparison = .{ .property = 2, .operator = .eq, .value = .{ .int = 7 } } } }, &hits, testing.allocator);
    try testing.expectEqual(@as(usize, 2), hits.items.len); // both pre-migration rows
    hits.clearRetainingCapacity();
    try query.where(&readTransaction, readTransaction.root(), .{ .predicate = .{ .comparison = .{ .property = 2, .operator = .eq, .value = .{ .int = 9 } } } }, &hits, testing.allocator);
