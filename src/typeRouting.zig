...
pub fn deleteNullifyX(txn: *WriteTxn, dir: Ref, type_id: u16, pk: u64, expected_version: u64) !DeleteResult {
    ...
    if (try isBlocked(txn, dir, type_id, okey)) return .blocked;
...
fn deleteWorker(txn: *WriteTxn, dir: Ref, type_id: u16, okey: u64, visited: *std.AutoHashMap(u64, void)) !Ref {
    ...
    const cat_t0 = try catalogRef(txn, dir, type_id);
    const pc = (try catalog.loadCatalog(txn, cat_t0)).prop_count;
    if ((try rows.getByObjectKey(txn, cat_t0, okey, rbuf[0..pc])) == null) return dir; // already gone
    const pk = rbuf[0];
    const schema = try snapshotSchema(txn, cat_t0, pc);
    ...
    cur = try nullifyInbound(txn, cur, type_id, okey);
    cur = try cleanOutbound(txn, cur, type_id, okey);
    return tombstoneAndReclaim(txn, cur, type_id, pk, okey, &schema);
}
...
// Phase 4) Tombstone (re-read current version, which matches in this txn), then
...
fn tombstoneAndReclaim(txn: *WriteTxn, dir: Ref, type_id: u16, pk: u64, okey: u64, schema: *const SchemaSnapshot) !Ref {
    ...
    var rbuf: [256]u64 = undefined;
    const t_cat = try catalogRef(txn, dir, type_id);
    const cur_ver = (try rows.getByObjectKey(txn, t_cat, okey, rbuf[0..pc])) orelse return dir;
    const dres = try rows.delete(txn, t_cat, pk, cur_ver);
    switch (dres) {
        .ok => |new_cat| {
            const cur = try setCatalogRef(txn, dir, type_id, new_cat);
            try rows.freeRowStorage(txn, schema.kinds[0..pc], schema.elems[0..pc], rbuf[0..pc]);
            return cur;
        },
        else => return dir,
    }
