...
pub fn listAppendInt(transaction: *WriteTransaction, catalogRef: Reference, primaryKey: u64, property: usize, value: u64) !Reference {
    const r = (try catalog.resolveProperty(transaction, catalogRef, primaryKey, property)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.propertyColumn, r.row);
    const new_root = try Column.append(transaction, old_root, value);
    return catalog.replaceCollRoot(transaction, catalogRef, r.row, property, new_root);
}
...
pub fn dictGet(transaction: anytype, catalogRef: Reference, primaryKey: u64, property: usize, key: []const u8) !?u64 {
    const r = (try catalog.resolveProperty(transaction, catalogRef, primaryKey, property)) orelse return error.NotFound;
    const dict_root = try Column.get(transaction, r.propertyColumn, r.row);
...
pub fn dictPut(transaction: *WriteTransaction, catalogRef: Reference, primaryKey: u64, property: usize, key: []const u8, val: u64) !Reference {
    const r = (try catalog.resolveProperty(transaction, catalogRef, primaryKey, property)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.propertyColumn, r.row);
