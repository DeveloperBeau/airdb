...
pub fn listLen(transaction: anytype, cat: Reference, pk: u64, prop: usize) !?u64 {
...
pub fn listGetInt(transaction: anytype, cat: Reference, pk: u64, prop: usize, index: u64) !u64 {
...
pub fn listGetBlob(transaction: anytype, cat: Reference, pk: u64, prop: usize, index: u64) ![]const u8 {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const list_root = try Column.get(transaction, r.prop_col, r.row);
...
pub fn listAppendInt(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, value: u64) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
    const new_root = try Column.append(transaction, old_root, value);
    return catalog.replaceCollRoot(transaction, cat, r.row, prop, new_root);
}

pub fn listSetInt(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, index: u64, value: u64) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
...
pub fn listAppendBlob(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, bytes: []const u8) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
...
pub fn setCountInt(transaction: anytype, cat: Reference, pk: u64, prop: usize) !?u64 {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return null;
    const set_root = try Column.get(transaction, r.prop_col, r.row);
    return try Index.count(transaction, set_root);
}

pub fn setContainsInt(transaction: anytype, cat: Reference, pk: u64, prop: usize, key: u64) !bool {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const set_root = try Column.get(transaction, r.prop_col, r.row);
    return (try Index.get(transaction, set_root, key)) != null;
}

pub fn setAddInt(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, key: u64) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
    if ((try Index.get(transaction, old_root, key)) != null) return cat; // already a member, no version bump
    const new_root = try Index.insert(transaction, old_root, key, 1);
    return catalog.replaceCollRoot(transaction, cat, r.row, prop, new_root);
}

pub fn setRemoveInt(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, key: u64) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
    if ((try Index.get(transaction, old_root, key)) == null) return cat; // not a member, no version bump
...
pub fn setCountBlob(transaction: anytype, cat: Reference, pk: u64, prop: usize) !?u64 {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return null;
    const set_root = try Column.get(transaction, r.prop_col, r.row);
    return try bindex.count(transaction, set_root);
}

pub fn setContainsBlob(transaction: anytype, cat: Reference, pk: u64, prop: usize, member: []const u8) !bool {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const set_root = try Column.get(transaction, r.prop_col, r.row);
    return (try bindex.get(transaction, set_root, member)) != null;
}

pub fn setAddBlob(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, member: []const u8) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
    if ((try bindex.get(transaction, old_root, member)) != null) return cat; // already a member, no version bump
    const new_root = try bindex.insert(transaction, old_root, member, 1);
    return catalog.replaceCollRoot(transaction, cat, r.row, prop, new_root);
}

pub fn setRemoveBlob(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, member: []const u8) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
    if ((try bindex.get(transaction, old_root, member)) == null) return cat; // not a member, no version bump
...
pub fn dictGet(transaction: anytype, cat: Reference, pk: u64, prop: usize, key: []const u8) !?u64 {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const dict_root = try Column.get(transaction, r.prop_col, r.row);
    return try bindex.get(transaction, dict_root, key);
}

pub fn dictCount(transaction: anytype, cat: Reference, pk: u64, prop: usize) !?u64 {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return null;
    const dict_root = try Column.get(transaction, r.prop_col, r.row);
    return try bindex.count(transaction, dict_root);
}

pub fn dictPut(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, key: []const u8, val: u64) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
...
pub fn dictRemove(transaction: *WriteTransaction, cat: Reference, pk: u64, prop: usize, key: []const u8) !Reference {
    const r = (try catalog.resolveProp(transaction, cat, pk, prop)) orelse return error.NotFound;
    const old_root = try Column.get(transaction, r.prop_col, r.row);
    if ((try bindex.get(transaction, old_root, key)) == null) return cat; // absent, no version bump
